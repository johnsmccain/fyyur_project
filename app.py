...
def venues():
  ...
  for venue in venues: 
    ...
    venueTemp.append(location)
  
  now =  datetime.now()
  for temp in venueTemp:
    venueList = []
    for venue in venues:
      if (venue.city == temp['city']) and (venue.state == temp['state']):
...
def search_venues():
  ...
  # Venue.query.filter(Venue.name.ilike)
...
def show_venue(venue_id):
  ...
  venues = Venue.query.all()
  for venue in venues:
  # shows = Show.query.filter_by(v)
    shows = Show.query.filter_by(venue_id=venue.id)
    past_shows_count = 0
    upcoming_shows_count = 0
    
    for count in shows:
      if count.start_time > now:
...
def create_venue_submission():
  ...
  # print(name,city,state, address,phone,genres,facebook_link, image_link,seeking_talent, seeking_description)

  genreList = ' '.join(map(str, genres))
...
def artists():
  ...
  allArtrist = Artist.query.all()
  data= []
  for one in allArtrist:
    data.append({
      "id": one.id,
      "name": one.name
    })
...
def edit_venue(venue_id):
  ...
  venue={
    "id": venue.id,
    "name": venue.name,
    "genres": ' '.join(map(str, venue.genres)),
    "address": venue.address,
    "city": venue.city,
    "state": venue.state,
    "phone": venue.phone,
...
def create_artist_submission():
  ...
  error_on_artist = False
  try:
    db.session.add(artist)
    db.session.commit()
  except:
    error_on_artist =True
    db.session.rollback()
  finally:
    db.session.close()
  if error_on_artist:
